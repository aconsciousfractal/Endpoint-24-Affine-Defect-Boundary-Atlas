...
| C11 | The public package is a published companion repository with tag `v1.0.1`; future release tags should repeat package checks and clean-clone verification. | CL1 release-status guardrail | `paper/PUBLIC_PACKAGE_MANIFEST.json`, `docs/RED_TEAM_REPORT.md` | public as release status | claiming future tags are ready without rerunning checks |
| C12 | The full endpoint control atlas is included only as a summary aggregate: 7040 square-mask pairs, endpoint distribution, T/K endpoint-orbit counts, selected-affine controls at endpoints 22/23/27, and global defect/selected-affine compatibility. | CL3 certified aggregate control | `results/FULL_ENDPOINT_CONTROL_ATLAS.json` | public as context for endpoint-24 scope | conceptual classification of every endpoint or full-payload all-endpoint export |
